...
vowels = ['A', 'E', 'I', 'O', 'U', 'Y']
vowels_with_labels = {vowel : i for i, vowel in enumerate(vowels)}
...
def load_test_data():
    ...
    dir = r'C:\Users\kunal\Documents\ME369P\Final_Project\vowel_test_images'
    ...
    for file in os.listdir(dir):
        ...
        label = vowels_with_labels[str(file[0])]
...
for i in range(len(random_test_labels)):
    actual_letter = list(vowels_with_labels.keys())[list(vowels_with_labels.values()).index(random_test_labels[i])]
    predicted_letter = list(vowels_with_labels.keys())[list(vowels_with_labels.values()).index(predicted_label[i])]
